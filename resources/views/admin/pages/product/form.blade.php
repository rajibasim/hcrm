        ...
        <div class="row">
          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              ...
              <!-- /.card-header -->
              <form id="dataForm" method="post" action="{{ isset($details->id) && $details->id ? route('product.update', $details->id) : route('product.store') }}" autocomplete="off" enctype="multipart/form-data">
                        ...
                        <select class="form-control select2" name="unit_id" id="unit_id" required="">
                          ...
                          @if($unit) && !$unit->isEmpty())
                            @foreach ( $unit as $key => $res )
                              <option value="{{ $res->id }}" {{ isset($details->unit_id) && $details->unit_id == $res->id ? 'selected' : '' }}>{{ $res->unit }}</option>
                            @endforeach
                          @endif
                        </select>
                      </div>
                        ...
                        <select class="form-control select2" name="category_id" id="category_id" required="">
                          ...
                          @if($category) && !$category->isEmpty())
                            @foreach ( $category as $key => $res )
                              <option value="{{ $res->id }}" {{ isset($details->category_id) && $details->category_id == $res->id ? 'selected' : '' }}>{{ $res->category }}</option>
                            @endforeach
                          @endif
                        </select>
                      </div>
