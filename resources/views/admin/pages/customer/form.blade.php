        ...
        <div class="row">
          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              ...
              <!-- /.card-header -->
               <form id="dataForm" method="post" action="{{ isset($details->id) && $details->id ? route('park_service.update', $details->id) : route('park_service.store') }}" autocomplete="off" enctype="multipart/form-data">
                        ...
                        <select class="form-control select2" name="service_id" id="service_id" required="">
                          ...
                          @if($service) && !$service->isEmpty())
                            @foreach ( $service as $key => $res )
                              <option value="{{ $res->id }}" data-price="{{ $res->price }}" data-duration="{{ $res->duration }}" {{ isset($details->service_id) && $details->service_id == $res->id ? 'selected' : '' }}>{{ $res->name }}</option>
                            @endforeach
                          @endif
                        </select>
                      </div>
                ...
                <div class="card-footer">
                  ...
                  <a href="{{ route('park_service.index') }}" class="btn btn-danger">Reset</a>
                </div>
              </form>
            </div>
            <!-- /.card -->
