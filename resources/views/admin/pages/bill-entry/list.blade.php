    ...
    <section class="content">
      <div class="container-fluid">
        <div class="row">
          <div class="col-md-12">
            <div class="card card-primary card-outline">
              @can('bill_entry_create')
                <div class="card-header">
                  <h3 class="card-title">{{ $metadata['page_title'] }}</h3>
                    <div class="float-right">
                        <a href="{{ route('bill-entry.create') }}" class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="top" title="New Records">
                ...
                <table class="table table-bordered">
                  ...
                  @if(isset($rows) && !$rows->isEmpty())
                    ...
                    <tr> 
                      ...
                      <td>{{ $res->offline_amount }}</td>
                      <td>{{ $res->balance_amount }}</td>
                      <td style="width: 130px;">
                        <!-- <a id="" href="{{ route('bill-entry.show',$res->id) }}" class="btn btn-warning btn-sm" data-toggle="tooltip" data-placement="top" title="View">
                          <i class="fa fa-eye" aria-hidden="true"></i>
                        </a> -->
                        @can('bill_entry_edit')
                          <a href="{{ route('bill-entry.edit',$res->id) }}" class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="top" title="Edit">
                            <i class="fas fa-edit" aria-hidden="true"></i>
                          </a>
                        @endcan
                        @can('bill_entry_delete')
                          <form id="deleteForm{{ $res->id }}" method="POST" action="{{ route('bill-entry.destroy', $res->id) }}" accept-charset="UTF-8" style="display:inline">
