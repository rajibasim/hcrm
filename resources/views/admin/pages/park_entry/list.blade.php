                ...
                <form method="get" action="" autocomplete="off" enctype="multipart/form-data">
                  <div class="row">
                    ...
                    <div class="col-sm-5">
                      <!-- select -->
                      <div class="form-group">
                        <select class="form-control select2" name="entry_id" id="entry_id">
                          <option value="">Select Entry</option>
                          @if($entry) && !$entry->isEmpty())
                            @foreach ( $entry as $key => $res )
                              <option value="{{ $res->id }}" {{ isset($serach_data['entry_id']) && $serach_data['entry_id'] == $res->id ? 'selected' : '' }}>{{ $res->name }}</option>
                            @endforeach
                          @endif
                        </select>
                      </div>
    ...
    <section class="content">
      <div class="container-fluid">
        <div class="row">
          <div class="col-md-12">
            <div class="card card-primary card-outline">
              @can('park_entry-create')
                <div class="card-header">
                  <h3 class="card-title">{{ $metadata['page_title'] }}</h3>
                    <div class="float-right">
                        <a href="{{ route('park_entry.create') }}" class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="top" title="New Records">
              ...
              <div class="card-body">
                <table class="table table-bordered">
                  ...
                  @if(isset($rows) && !$rows->isEmpty())
                    ...
                    <tr> 
                      <td>{{ $res->park->name }}</td>
                      <td>{{ $res->entry->name }}</td>
                      <td>{{ $res->price }}</td>
                      <td>{{ $res->is_active == 1 ? 'Active' : 'In-Active' }}</td>
                      <td style="width: 100px;">
                        @can('park_entry-edit')
                          <a href="{{ route('park_entry.edit',$res->id) }}" class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="top" title="Edit">
                            <i class="fas fa-edit" aria-hidden="true"></i>
                          </a>
                        @endcan
                        @can('park_entry-delete')
                          <form id="deleteForm{{ $res->id }}" method="POST" action="{{ route('park_entry.destroy', $res->id) }}" accept-charset="UTF-8" style="display:inline">
