                ...
                <form method="get" action="" autocomplete="off" enctype="multipart/form-data">
                  <div class="row">
                    ...
                    <div class="col-sm-4">
                      <!-- select -->
                      <div class="form-group">
                        <input type="text" class="form-control" placeholder="Contact Email" name="email" value="{{ isset($serach_data['email']) && $serach_data['email'] ? $serach_data['email'] : '' }}">
                      </div>
                    </div>
                    <div class="col-sm-4">
                      <!-- select -->
                      <div class="form-group">
                        <input type="text" class="form-control" placeholder="Contact Phone" name="phone" value="{{ isset($serach_data['phone']) && $serach_data['phone'] ? $serach_data['phone'] : '' }}">
                      </div>
                    </div>
                    <div class="col-4">
                        <div class="form-group">
                          <input type="text" class="form-control" placeholder="Pin Code" name="zip_code" value="{{ isset($serach_data['zip_code']) && $serach_data['zip_code'] ? $serach_data['zip_code'] : '' }}">
                        </div>
                    </div>
                    <div class="col-1">
                ...
                <div class="card-header">
                  <h3 class="card-title">{{ $metadata['page_title'] }}</h3>
                    <div class="float-right">
                        <a href="{{ route('parks.create') }}" class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="top" title="New Records">
                ...
                <table class="table table-bordered">
                  ...
                  @if(isset($rows) && !$rows->isEmpty())
                    ...
                    <tr> 
                      <td>
                        @if($res->banner)
                          <img src="{{ asset('storage/banner/'.$res->banner) }}" alt="#ProfileImage" width="50px" height="50px"></td>
                        @else
                      ...
                      <td style="width: 130px;">
                        <a href="{{ route('parks.show',$res->id) }}" class="btn btn-primary btn-sm" data-toggle="tooltip" data-placement="top" title="View">
                            <i class="fas fa-eye" aria-hidden="true"></i>
                        </a>
                        @can('park-edit') 
                          <a href="{{ route('parks.edit',$res->id) }}" class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="top" title="Edit">
                            <i class="fas fa-edit" aria-hidden="true"></i>
                          </a>
                        @endcan
                        @can('park-delete') 
                          <form id="deleteForm{{ $res->id }}" method="POST" action="{{ route('parks.destroy', $res->id) }}" accept-charset="UTF-8" style="display:inline">
