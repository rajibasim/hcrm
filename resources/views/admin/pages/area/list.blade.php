            ...
            <div class="card card-primary card-outline">
              ...
              <div class="card-body">
                <form method="get" action="" autocomplete="off" enctype="multipart/form-data">
                  <div class="row">
                    <div class="col-5">
                      <select class="form-control select2" name="beat_id" id="beat_id">
                    ...
                    <div class="col-5">
                      <input type="text" class="form-control" placeholder="Area" name="area" value="{{ isset($serach_data['area']) && $serach_data['area'] ? $serach_data['area'] : '' }}">
                    </div>
                    <div class="col-1">
                ...
                <div class="card-header">
                  <h3 class="card-title">{{ $metadata['page_title'] }}</h3>
                    <div class="float-right">
                        <a href="{{ route('area.create') }}" class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="top" title="New Records">
              ...
              <div class="card-body">
                <table class="table table-bordered">
                  ...
                  @if(isset($rows) && !$rows->isEmpty())
                    ...
                    <tr> 
                      <td>{{ $res->area }}</td>
                      <td>{{ $res->beats->beat }}</td>
                      <td>{{ $res->is_active == 1 ? 'Active' : 'In-Active' }}</td>
                      <td style="width: 100px;">
                        @can('area_create')
                          <a href="{{ route('area.edit',$res->id) }}" class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="top" title="Edit">
                            <i class="fas fa-edit" aria-hidden="true"></i>
                          </a>
                        @endcan
                        @can('area_delete')
                          <form id="deleteForm{{ $res->id }}" method="POST" action="{{ route('area.destroy', $res->id) }}" accept-charset="UTF-8" style="display:inline">
