                        ...
                        <select class="form-control select2" name="activity_id" id="activity_id">
                          ...
                          @if($activity) && !$activity->isEmpty())
                            @foreach ( $activity as $key => $res )
                              <option value="{{ $res->id }}" {{ isset($serach_data['activity_id']) && $serach_data['activity_id'] == $res->id ? 'selected' : '' }}>{{ $res->name }}</option>
                            @endforeach
                          @endif
                        </select>
                      </div>
    ...
    <section class="content">
      <div class="container-fluid">
        <div class="row">
          <div class="col-md-12">
            <div class="card card-primary card-outline">
              @can('park_activity-create')
                <div class="card-header">
                  <h3 class="card-title">{{ $metadata['page_title'] }}</h3>
                    <div class="float-right">
                        <a href="{{ route('park_activity.create') }}" class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="top" title="New Records">
                  ...
                  @if(isset($rows) && !$rows->isEmpty())
                    ...
                    <tr> 
                      ...
                      <td style="width: 100px;">
                        @can('park_activity-edit')
                          <a href="{{ route('park_activity.edit',$res->id) }}" class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="top" title="Edit">
                            <i class="fas fa-edit" aria-hidden="true"></i>
                          </a>
                        @endcan
                        @can('park_activity-delete')
                          <form id="deleteForm{{ $res->id }}" method="POST" action="{{ route('park_activity.destroy', $res->id) }}" accept-charset="UTF-8" style="display:inline">
